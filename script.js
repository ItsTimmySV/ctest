import { getCards, saveCards, formatCurrency, formatDate, setupModalClose, getAlertSettings, saveAlertSettings, wasAlertSent, markAlertAsSent } from './utils.js';
...
import { getNotificationsForCards, showBrowserNotification } from './alert-system.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NEW: Alert settings elements
    const alertDaysAdvanceInput = document.getElementById('alert-days-advance');
    const alertPaymentDueCheckbox = document.getElementById('alert-payment-due');
    ...
    const updateUI = () => {
        ...
        // NEW: Check and display alerts after UI update
        // We delay slightly to ensure DOM is settled if needed, though not strictly critical for notifications
        setTimeout(() => {
            checkAndDisplayNotifications();
        }, 100);
    };

    // NEW: Notification checking and display function
    const checkAndDisplayNotifications = () => {
        const notifications = getNotificationsForCards(cards);
        notifications.forEach(notification => {
            showBrowserNotification(notification);
        });
    };

    // NEW: Load and apply alert settings
    const loadAlertSettings = () => {
        const settings = getAlertSettings();
        if (alertDaysAdvanceInput) alertDaysAdvanceInput.value = settings.daysAdvance;
        if (alertPaymentDueCheckbox) alertPaymentDueCheckbox.checked = settings.enablePaymentDue;
    };

    // NEW: Save alert settings
    const handleSaveAlertSettings = () => {
        const settings = {
            daysAdvance: parseInt(alertDaysAdvanceInput.value) || 3, // Default to 3 days
            enablePaymentDue: alertPaymentDueCheckbox.checked,
            enableCutoffDate: false // This feature is not implemented/used
        };
    ...
    // NEW: Add event listener for sidebar add card button (mobile)
    ...
    // NEW: Alert settings event listeners
    saveAlertSettingsBtn?.addEventListener('click', handleSaveAlertSettings);
    ...
    if ('Notification' in window) {
        Notification.requestPermission().then(permission => {
            if (permission === 'granted') {
                ...
                // If permission is granted, register service worker and check for notifications
    ...
    loadAlertSettings(); // NEW: Load alert settings
    updateUI();
});